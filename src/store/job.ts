...
export async function refreshJobs() {
  ...
  for (let i = 0; i < jobs.length; i += 1) {
    const job = new TaskCore({ id: jobs[i] });
...
/** 向异步任务队列中添加任务 */
export function appendJob(job: TaskCore) {
...
/** 从异步任务队列中移除指定任务 */
export function removeJob(job: TaskCore) {
...
export function createJob(body: {
  ...
  const job = new TaskCore({ id: job_id });
  job.onFinish(() => {
    refreshJobs();
    if (onFinish) {
      onFinish();
